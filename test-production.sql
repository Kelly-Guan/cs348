...
-- psql postgres://postgres:postgres@127.0.0.1:5432 -f test-production.sql > test-production.out
-- This will run the tests fresh and put the results in test-production.out
...
SELECT * FROM users u WHERE u.username = 'baconman';
...
/* Should select all of Reviewer 100's ratings */
SELECT
  m.title,
  r.*,
  rv.upvotes,
...
/* Should return the average rating for a movie (Mid 10) */
...
/* Should return the rating ranking for a movie, i.e. the number of rows with average score above it (Mid 10) */
WITH mid_rating AS (
...
/* Should return all the rating details for a film (Mid 10) */
...
/* Should return all the movie details for a film excluding cast (Mid 10) */
SELECT release_date, poster_link, runtime, adult
FROM movies
WHERE mid = 10
LIMIT 10;

/* Should return all the writers for a movie (Mid 10) */
SELECT name, role
FROM movie_cast
WHERE mid = 10
  AND role = 'crew'
...
/* Should return all the actors for a movie (Mid 10) */
