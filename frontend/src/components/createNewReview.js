...
function CreateNewReview({ mid, movieImgVert, movieTitle, onClose, onSubmit }) {
  ...
  const handleSubmit = async () => {
    ...
    const reviewData = {
      ...
      date_posted: date.toISOString(), // Format date to ISO string
    };

    console.log("Review Data:", reviewData);

    try {
      const response = await fetch('http://localhost:3001/api/ratings/add-review', {
        method: 'POST',
        headers: {
      ...
      <div className="popup-content bg-white rounded-lg relative flex flex-col w-2/5 p-8">
        ...
        <div className="flex ">
          <div className="w-1/2 px-5 ">
            <img
              className="w-full relative top-0 object-cover rounded mb-4"
              src={movieImgVert}
              alt={movieTitle}
            />
          </div>
        ...
        <button
          className="bg-pink-500 text-white py-2 px-4 rounded self-end m-5"
